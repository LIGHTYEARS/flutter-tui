...
Flutter-TUI 通过 SGR 鼠标协议支持鼠标点击、移动、滚轮等交互。鼠标事件经过命中测试（hit-test）定位到具体的渲染对象，并通过 `MouseRegion` widget 分发到用户代码。
...
Flutter-TUI 使用 SGR（Select Graphic Rendition）扩展鼠标协议，通过以下转义序列启用：
